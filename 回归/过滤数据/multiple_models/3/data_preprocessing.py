# 该文件是用来对三个模型共存情况的数据进行预处理的
# 主要预处理的部分是，对每一次三模型共存的运行，整理出三个模型在单位时间内的多个与L2相关的数据交换量
# 该文件需要读取三个模型的共存执行数据，即tmp_d_r_v.csv, 将输出res_tmp_d_r_v.csv等文件作为结果
# res_tmp_d_r_v.csv文件中会在每一行共存执行结果后面附加每个模型执行过程中与L2相关的四个数据交换量的归一化值

import collections
import csv

data_file = ["tmp_d_r_v.csv"]
...
for file in data_file:
    with open(file, mode="r", encoding="utf-8-sig") as f:
        ...
        for row in reader:
            ...
            m1_name, m2_name, m3_name = row[0], row[8], row[16]
            m1_gpu, m2_gpu, m3_gpu = row[1], row[9], row[17]
            if m1_gpu == "90" or m2_gpu == "90" or m3_gpu == "90":
                continue
            m1_bs, m2_bs, m3_bs = row[2], row[10], row[18]
            if m1_bs == "4" or m2_bs == "4" or m3_bs == "4":
                continue
            m1_latency, m2_latency, m3_latency = row[3], row[11], row[19]
            m1_increase, m2_increase, m3_increase = row[5], row[13], row[21]
            m1_increase_percent, m2_increase_precent, m3_increase_precent = row[6], row[14], row[22]
            # 以上数据都是字符串类型
    ...
    # break
            #----------------------保存结果部分-----------------------
            # 因为多个模型共存时，每条共存数据可以让任意模型作为预测模型，所以在三模型共存时每条共存数据可以产生三个输入数据
            # 保存预处理结果时，每条共存数据
            # 让第一个模型作为目标预测模型的数据:
            predicted_model_data = L1_results
            colocated_model_data = []
            for i in range(4):
                colocated_model_data.append(L2_results[i] + L3_results[i])
            ...
            colocated_model_data = []
            for i in range(4):
                colocated_model_data.append(L1_results[i] + L3_results[i])
            ...
            colocated_model_data = []
            for i in range(4):
                colocated_model_data.append(L1_results[i] + L2_results[i])
