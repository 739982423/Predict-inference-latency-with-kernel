...
if __name__ == "__main__":
    ...
    # 临时文件名称，用来存储4个子进程的运行结果，在主进程中读取每次运行的结果，进行排序后再存入最终的csv文件
    ...
    p4 = Process(target=run, args=(args.model4, barrier, args.batch4, args.gpu4, model_exec_times, tmp_csv_name))
    # # 准备保存结果的文件
    # title = ["model name", "gpu resource", "batchsize", "total latency", "real exec times", "avg latency"]
