...
class QLearningAgent(object):
    ...
    def learn(self, state, action, reward, next_state, next_action, done):
        ...
        if done:
            ...
            # QLearning与SARSA相比，只有更新函数的targetQ部分有所改变:
            # FROM:
            # target_Q = reward + self.gamma * self.Q[next_state, next_action]
            # TO:
            target_Q = reward + self.gamma * max(self.Q[next_state, :])
            # 另外SARSA中需要的next_action这个参数，在QLearning中也不需要使用了
...
def train_episode(env, agent, is_render):
    ...
    while True:
        ...
        # ---------- Qlearning中更新Q表格不需要下一个时刻的action了，所以应该省略 -----------
        ...
        # -------- 注释掉action的更新操作，因为我们把52行放进while内了，每次循环都重新随机选择action ------
        # ---------------- sarsa中需要把当前步骤找到的下一步action切实作为下一步action ----------------
        # action = next_action
        # -------------------------------------------------------------------------------------
