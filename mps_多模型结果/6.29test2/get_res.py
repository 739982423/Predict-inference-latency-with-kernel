        ...
        with open(target_csv_name, mode="a", encoding="utf-8-sig", newline="") as f:
            ...
            for pair in tmp_res:
                ...
                if len(pair[1]) >= 3:
                    ...
                    extra_data.append("")
                    ratio = float(pair[0][4]) / float(pair[1][4])
                    extra_data.append(ratio)
                    extra_data.append("")

                    tmp = memory_message_hash[key_name][:]
                    for i in range(len(tmp)):
                        extra_data.append(float(tmp[i]) / float(ratio))
