...
with open(input_csv, mode="r", encoding="utf-8-sig") as f:
    ...
    for row in reader:
        ...
        m4_g = int(row[25])
        m4_b = int(row[26])
        m4_looptimes = int(row[28])
        m4_latency_increase = float(row[30]) * 100
        ...
        base_m4_data_transfer = m_data[m4_name + "_b" + str(m4_b) + "_g" + str(m4_g)]
        ...
        # m4归一化后的数据传输量
        m4_ratio = m4_looptimes / m1_looptimes
        normalized_m4_data_transfer = base_m4_data_transfer * m4_ratio
        # print("normalized m3 数据传输量:{}".format(normalized_m3_data_transfer))

        x0 = normalized_m2_data_transfer + normalized_m3_data_transfer + normalized_m4_data_transfer
        ...
        # m4归一化后的数据传输量
        m4_ratio = m4_looptimes / m3_looptimes
        normalized_m4_data_transfer = base_m4_data_transfer * m4_ratio
        # print("normalized m2 数据传输量:{}".format(normalized_m2_data_transfer))

        x0 = normalized_m1_data_transfer + normalized_m2_data_transfer + normalized_m4_data_transfer
        ...
        # predict m4
        # m1归一化后的数据传输量
        m1_ratio = m1_looptimes / m4_looptimes
        normalized_m1_data_transfer = base_m1_data_transfer * m1_ratio
        # print("normalized m1 数据传输量:{}".format(normalized_m1_data_transfer))

        # m2归一化后的数据传输量
        m2_ratio = m2_looptimes / m4_looptimes
        normalized_m2_data_transfer = base_m2_data_transfer * m2_ratio
        # print("normalized m3 数据传输量:{}".format(normalized_m3_data_transfer))

        # m3归一化后的数据传输量
        m3_ratio = m3_looptimes / m4_looptimes
        normalized_m3_data_transfer = base_m3_data_transfer * m3_ratio
        # print("normalized m3 数据传输量:{}".format(normalized_m3_data_transfer))

        x0 = normalized_m1_data_transfer + normalized_m2_data_transfer + normalized_m3_data_transfer
        x1 = base_m4_data_transfer
        x2 = m4_g
        coff = eval(m4_name + "_coefficient")
        predict = func_4dimension([x0, x1, x2], coff[0], coff[1], coff[2], coff[3], coff[4])
        MAE4 += abs(predict - m4_latency_increase)
        print("predict m4:{}, real m4:{}, g4:{}".format(predict, m4_latency_increase, m4_g))
